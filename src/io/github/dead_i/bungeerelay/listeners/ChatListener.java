...
public class ChatListener implements Listener {
    ...
    public void onChat(ChatEvent event) {
        ...
        if (!msg.startsWith("/") || (msg.startsWith("/me") && p.hasPermission("irc.me"))) {
            String chanconf = IRC.config.getString("server.channel");
            String channel = chanconf;
            if (channel.isEmpty()) channel = IRC.config.getString("server.chanprefix") + p.getServer().getInfo().getName();
            ...
            if (!chanconf.isEmpty()) for (ProxiedPlayer o : plugin.getProxy().getPlayers()) {
                if (!p.getServer().getInfo().getName().equals(o.getServer().getInfo().getName())) o.sendMessage(new TextComponent(ChatColor.translateAlternateColorCodes('&', IRC.config.getString("formats.msg"))
