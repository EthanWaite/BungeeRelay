...
public class Channel {
    ...
    public boolean isBanned(ProxiedPlayer p) {
        for (String ban : bans) {
            ...
            if ((nick.equalsIgnoreCase(BungeeRelay.getConfig().getString("server.userprefix") + p.getName() + BungeeRelay.getConfig().getString("server.usersuffix")) || nick.equals("*")) && (host.equals(userhost) || host.equals("*")) && (ident.equalsIgnoreCase(p.getName()) || ident.equals("*"))) return true;
        }
        return false;
    }
}
